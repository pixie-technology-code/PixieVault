...
- **2026-09-02**: Immediate Commit Rule for Multi-Collaborator Repositories ("Please commit changes to git after each change. There are multiple people editing this repo").
  - **Rule**: Commit and push changes to git immediately after each atomic modification or task step, rather than batching them across conversational turns. Always verify remote status to prevent merge conflicts in active multi-collaborator branches.
