...
- **2026-09-01**: Native Composer Service Healthcheck and Cross-Platform Python Execution ("Healthcheck failed for service 'backend': Timed out").
  - **Rule**: 
    ...
    4. **Automated Live Testing**: Maintain an automated live service orchestration test (`test-mikrotik-live-host.js` and `test_vault_composer_launches_mikrotik_fleet_live`) in `./test-all.sh`, `./test-all.ps1`, and `./test-all.bat`.
